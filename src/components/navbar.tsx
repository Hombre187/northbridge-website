...
export function Navbar() {
  ...
  return <header className={`fixed inset-x-0 top-0 z-50 border-b transition ${solid || open ? "border-white/10 bg-[#08080af2] backdrop-blur-xl" : "border-transparent bg-transparent"}`}>
    <Container className="flex h-20 items-center justify-between"><Link href="/" className="flex items-center gap-3" aria-label="NorthBridge RP Home"><Image src="/branding/nb-logo.webp" alt="NorthBridge RP" width={717} height={479} priority fetchPriority="high" sizes="(max-width: 639px) 48px, 60px" className="h-8 w-auto shrink-0 object-contain sm:h-10"/><span className="font-black tracking-[.12em]">NORTHBRIDGE <span className="text-[#D71920]">RP</span></span></Link>
